...
QEMU_COMMON_FLAGS=-m 256 -cpu core2duo -smp 1 -device virtio-serial,id=nxtoolsBus0 -device virtserialport,chardev=nxtoolsChardev0,name=kata,id=nxtoolsGuest0 -chardev socket,path=/tmp/GOV,server,nowait,id=nxtoolsChardev0 -chardev socket,id=cmsg1,path=/tmp/cmsg,server,nowait -mon chardev=cmsg1,mode=control --drive file=image_file.raw,format=raw,if=virtio
...
#for debug
#-boot d -kernel kernel.elf -m 256 -cpu core2duo -smp 1 -device virtio-serial,id=nxtoolsBus0 -device virtserialport,chardev=nxtoolsChardev0,name=nxtools0,id=nxtoolsGuest0 -chardev socket,path=/tmp/GOV,server,id=nxtoolsChardev0 -serial stdio

#{ "execute": "device_add", "arguments": { "driver":"virtserialport", "chardev": "nxtoolsChardev1", "name": "kata1", "id": "nxtoolsGuest1", "bus":"nxtoolsBus0.0"}}

#{"execute": "chardev-add", "arguments": {"id":"nxtoolsChardev1", "backend": {"type": "socket", "data":{"addr": {"type": "unix", "data": {"path": "/tmp/kata1"}}, "wait": false, "server": true}} }}
