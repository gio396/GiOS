...
QEMU_COMMON_FLAGS=-m 256 -cpu core2duo -smp 1 -device virtio-serial,id=nxtoolsBus0 -device virtserialport,chardev=nxtoolsChardev0,name=kata,id=nxtoolsGuest0 -chardev socket,path=/tmp/GOV,server,nowait,id=nxtoolsChardev0 -chardev socket,id=cmsg1,path=/tmp/cmsg,server,nowait -mon chardev=cmsg1,mode=control --drive file=image_file.raw,if=virtio
